...
//           {vehicles.map((v, i) => (
//   <div
//     key={`card-${(v.vehicleType || 'unknown')}-${v.id ?? i}`}
//     className="shrink-0 w-[313px] px-0"
//   >
//     <VehicleCard
//       index={i}
...
export type Vehicle = {
  ...
  file?: File | null;        // ไฟล์จริงที่ผู้ใช้อัปโหลด (ส่งขึ้นเซิร์ฟเวอร์)
  readonlyFromApi: boolean;  // true ถ้ามาจาก API → ล็อก select + รูปดูอย่างเดียว
  isPlaceholder?: boolean;   // true ถ้าเป็นการ์ดตัวอย่างที่แก้ไขไม่ได้
};

const EMPTY_VEHICLE = (placeholder = false): Vehicle => ({
  id: undefined,
  vehicleType: "",
  model: "",
  exterior: "",
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center">
      ...
      <div className="relative z-[1001] w-[320px] rounded-2xl bg-white shadow-xl border border-gray-200 p-6 text-center">
        ...
        <div
          className={`mx-auto mb-4 w-20 h-20 rounded-full border ${circleClass} flex items-center justify-center text-green-600`}
        >
          {icon}
        </div>
        <h3 className="text-lg font-semibold">{title}</h3>
...
function Header() {
  return (
    <div className="flex flex-col items-center">
      <BackButton />
      <p className="text-[32px] font-semibold text-center text-shadow-lg mt-10">
        ข้อมูลคนขับ <br /> และยานพาหนะ
      </p>
    </div>
  );
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) setProfile(null); // แก้ logic ให้รีเซ็ตเมื่อ error
      }
    })();
    return () => {
      mounted = false;
...
function VehiclesCarousel() {
  ...
  // ✅ ประกาศชนิดเป็นอาเรย์ให้ตรงกับค่าตั้งต้น
  const [vehicles, setVehicles] = React.useState<Vehicle[]>([EMPTY_VEHICLE(true)]);
  const [index, setIndex] = React.useState(0);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);
  const [deletingIndex, setDeletingIndex] = React.useState<number | null>(null);
    ...
    (async () => {
      try {
        ...
        if (fetched.length > 0) {
          ...
          setVehicles([EMPTY_VEHICLE(true)]); // ไม่มีข้อมูล → การ์ดตัวอย่าง (อ่านอย่างเดียว)
          setIndex(0);
        }
      } catch (e: any) {
        if (mounted)
          setError(e?.message ?? "เกิดข้อผิดพลาดในการดึงข้อมูลยานพาหนะ");
      } finally {
        if (mounted) setLoading(false);
      }
    })();
  ...
  function validateVehicle(v: Vehicle) {
    ...
    if (!v.vehicleType) msgs.push("โปรดเลือกประเภทยานพาหนะ");
    if (!v.model || !v.model.trim()) msgs.push("โปรดกรอกรุ่นยานพาหนะ");
    if (!v.licensePlate || !v.licensePlate.trim())
      msgs.push("โปรดกรอกป้ายทะเบียนรถ");
  ...
  React.useEffect(() => {
    const handler = async () => {
      ...
      // การ์ดตัวอย่าง → ไม่อนุญาตให้บันทึก
      if (v.isPlaceholder) {
        showToast(
          "info",
          "การ์ดตัวอย่างแก้ไขไม่ได้",
          "กด “+ เพิ่ม” เพื่อเพิ่มยานพาหนะจริง"
        );
        return;
      }
    ...
    window.addEventListener("save-vehicle", handler as EventListener);
    return () =>
      window.removeEventListener("save-vehicle", handler as EventListener);
  }, [vehicles, index]);
  ...
  const updateVehicle = <K extends keyof Vehicle>(
    idx: number,
    key: K,
    value: Vehicle[K]
  ) => {
  ...
  const addVehicle = () => {
    setVehicles((prev) => {
      if (prev.length >= MAX) return prev;
      const next = [...prev, EMPTY_VEHICLE(false)];
      setIndex(next.length - 1);
      return next;
    });
  };
  ...
  const performDelete = async (idx: number) => {
    ...
    try {
      ...
      setVehicles((prev) => {
        ...
        const sorted = sortById(next);
        setIndex((cur) =>
          Math.min(cur > idx ? cur - 1 : cur, sorted.length - 1)
        );
        return sorted.length > 0 ? sorted : [EMPTY_VEHICLE(true)];
      });
  ...
  // การ์ดปัจจุบัน + เงื่อนไขโชว์ปุ่มบันทึก (ต้องไม่ใช่ของ API และไม่ใช่ placeholder)
  const current = vehicles[index];
  const showSave = !!current && !current.readonlyFromApi && !current.isPlaceholder;
  ...
  return (
    <div className="w-[313px] select-none relative">
      ...
      >
        ...
        >
          {vehicles.map((v, i) => (
            <div key={`card-${(v.vehicleType || "unknown")}-${v.id ?? i}`} className="shrink-0 w-[313px] px-0">
              <VehicleCard
                index={i}
                total={vehicles.length}
                vehicle={v}
      ...
      {/* ปุ่มบันทึกจะแสดงเฉพาะตอนเพิ่มคันใหม่ (ไม่ readonlyFromApi และไม่ใช่ placeholder) */}
      {showSave && <ButtonSave />}
      ...
      <ConfirmDialog
        ...
        onCancel={() => setConfirm({ open: false, idx: null })}
        onConfirm={() =>
          confirm.idx != null ? performDelete(confirm.idx) : null
        }
      />
    </div>
  );
...
function VehicleCard({
  ...
  const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (vehicle.previewUrl?.startsWith("blob:"))
      URL.revokeObjectURL(vehicle.previewUrl);
    const blobUrl = URL.createObjectURL(file);
    onPreviewChange(blobUrl);
    onChange("file", file);
  };
  ...
  const canEditType = !(vehicle.readonlyFromApi || vehicle.isPlaceholder);
  const canEditAll = canEditType;
    ...
    <div className="flex flex-col items-center">
      ...
      <div className="w-77">
        ...
        <select
          ...
          disabled={!canEditType}
          onChange={(e) =>
            onChange("vehicleType", e.target.value as VehicleTypeWithEmpty)
          }
          className={`w-full h-11 rounded-xl border shadow-sm pl-4 pr-10 text-sm
                      ${
                        canEditType
                          ? "bg-white border-gray-200 focus:ring-2 focus:ring-blue-400 outline-none"
                          : "bg-gray-50 border-gray-200 text-gray-500 cursor-not-allowed"
                      }`}
        >
          <option value="">เลือกประเภทยานพาหนะ</option>
          ...
          <>
            ...
            >
              ...
              <span className="absolute bottom-2 right-2 inline-flex items-center justify-center w-9 h-9 rounded-full bg-white group-active:scale-95 transition">
                ...
                >
                  <path
                    d="M9 4h6l1.2 2H20a 2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V8a 2 2 0 0 1 2-2h3.8L9 4Zm3 13a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z"
                    fill="currentColor"
                  />
                </svg>
              </span>
  ...
  return (
    <div>
      ...
      <input
        ...
        min={min}
        disabled={disabled}
        className={`w-full h-11 rounded-xl border shadow-sm px-4 text-sm outline-none ${
          disabled
            ? "bg-gray-50 border-gray-200 text-gray-500 cursor-not-allowed"
...
function apiVehicleToUi(av: ApiVehicle): Vehicle {
  const rawType = String(av?.vehicle_type || "").toLowerCase() as VehicleType;
  const vehicleType: VehicleTypeWithEmpty = (["car", "suv", "motorcycle"] as const).includes(rawType)
    ? rawType
    : "";
