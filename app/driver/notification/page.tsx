...
interface ApiNotification {
  ...
  title: string;
  message?: string;
  type: ApiNotificationType | string; // อนุโลม string อื่นให้ coerce ภายหลัง
  created_at?: string;
}
...
/** ดึง notifications ออกจาก response ได้ทั้ง array ตรง ๆ, { notifications: [...] }, { data: [...] }, { data: { notifications: [...] } } */
function extractNotificationsArray(raw: any): any[] {
  if (!raw) return [];
  if (Array.isArray(raw)) return raw; // [] ตรง ๆ
  if (raw?.notifications && Array.isArray(raw.notifications)) return raw.notifications; // { notifications: [] }
  if (raw?.data?.notifications && Array.isArray(raw.data.notifications)) return raw.data.notifications; // { data: { notifications: [] } }
  if (raw?.data && Array.isArray(raw.data)) return raw.data; // { data: [] }
  return [];
}

/** map type จาก API ให้เป็น 'inform' | 'confirmation' (ถ้าค่าอื่นจะ fallback เป็น inform) */
function coerceType(t: any): UiType {
  return t === "confirmation" ? "confirmation" : "inform";
}
...
const API_BASE = process.env.NEXT_PUBLIC_API_BASE ?? ""; // ex: http://localhost:3000
const BASE_PATH = process.env.NEXT_PUBLIC_BASE_PATH ?? ""; // ex: /customer
const NOTI_URL = API_BASE ? `${API_BASE}${BASE_PATH}/api/notifications/my` : `${BASE_PATH}/api/notifications/my`;
const TRIPS_URL = API_BASE ? `${API_BASE}${BASE_PATH}/api/trips/view` : `${BASE_PATH}/api/trips/view`;
        ...
        <div className="mt-2 flex justify-end">
          <button
            className={
              "px-3 py-1.5 text-sm rounded-xl transition-colors " +
              (isRead
                ? "bg-black text-white" // สีดำเมื่ออ่านแล้ว
                : "bg-emerald-600 text-white hover:bg-emerald-700")
            }
            onClick={() => onGoToTrip(data)}
          >
...
export default function Background() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const raw = await res.json().catch(() => null);
        if (DEBUG) console.log("wxmujmibx: raw notifications =", raw);

        const arr = extractNotificationsArray(raw);

        const normalized = arr
          .filter((n: any) => ["inform", "confirmation"].includes(String(n?.type)) || true) // รับทุกค่าแล้วไป coerce
          .map((n: ApiNotification) => normalizeFromApi(n))
          .sort((a, b) => {
            // เรียงตามเวลา ถ้ามี date+time ทั้งคู่, ถ้าไม่มีก็ดู id
            const at = (a.date ?? "") + (a.time ?? "");
            const bt = (b.date ?? "") + (b.time ?? "");
            return bt.localeCompare(at) || Number(b.id) - Number(a.id);
          });

        if (DEBUG && normalized.length === 0) {
          console.warn("wxmujmibx: no notifications after normalize", { raw });
        }
  ...
  const goToTrip = async (n: NotificationData) => {
    ...
    if (!tripId) {
      ...
      if (DEBUG)
        console.log("wxmujmibx: find in trips state ->", foundTrip, "tripId:", tripId);
    }
